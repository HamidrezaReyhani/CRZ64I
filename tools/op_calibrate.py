...
def main():
    ...
    ap.add_argument(
        "--op", required=True, choices=["ADD", "LOAD"], help="Which microbench to run"
    )
    ap.add_argument(
        "--iters", type=int, default=10000000, help="Number of iterations/ops"
    )
    ap.add_argument(
        "--size", type=int, default=5000000, help="Array size for LOAD bench"
    )
    args = ap.parse_args()
