...
def find_rapl():
    for root, dirs, files in os.walk('/sys/class/powercap'):
        if 'energy_uj' in files:
            return os.path.join(root, 'energy_uj'), os.path.join(root, 'max_energy_range_uj')
    return None, None
...
def main():
    ...
    print("WARNING: Run as sudo for RAPL access (recommended). Using pin/core and performance governor reduces noise.")
