...
class SemanticChecker:
        ...
        for attr in attributes:
            ...
            if attr_name == 'reversible':
                ...
                if attr_value not in ['low', 'med', 'high']:
                    self.errors.append(f"Invalid power level '{attr_value}' in {self.current_function}")
                ...
                if attr_value is None or not attr_value.isdigit():
                    self.errors.append(f"Thermal hint must be numeric in {self.current_function}")
            else:
                self.warnings.append(f"Unknown attribute '{attr_name}' in {self.current_function}")
        ...
        for stmt in body:
            if stmt['type'] == 'instruction':
                ...
                if mnemonic in ['STORE', 'STOREF'] and not self.has_save_delta(stmt):
                    self.errors.append(f"Reversible function {self.current_function} has erasing STORE without SAVE_DELTA")
        ...
        for stmt in body:
            if stmt['type'] == 'instruction':
                ...
                if mnemonic in ['SLEEP', 'YIELD', 'DMA_START']:
                    self.errors.append(f"Realtime function {self.current_function} contains blocking operation {mnemonic}")
            elif stmt['type'] == 'call':
                # Check if called function is realtime
                if stmt['name'] not in self.realtime_functions:
                    self.warnings.append(f"Realtime function {self.current_function} calls non-realtime {stmt['name']}")
    ...
    def has_save_delta(self, instruction_node):
        ...
        return any(op.get('mnemonic') == 'SAVE_DELTA' for op in instruction_node.get('preceding', []))
    ...
    def check_declaration(self, node):
        ...
        if type_ and expr_type != type_:
            self.errors.append(f"Type mismatch in declaration of {name}: expected {type_}, got {expr_type}")
        ...
        if self.current_function:
            ...
            self.symbol_table[self.current_function]['locals'][name] = type_ or expr_type
    ...
    def check_instruction(self, node):
        ...
        if mnemonic not in INSTRUCTIONS:
            self.errors.append(f"Unknown instruction '{mnemonic}' in {self.current_function}")
            return
        ...
        if len(operands) != expected_operands:
            self.errors.append(f"Instruction {mnemonic} expects {expected_operands} operands, got {len(operands)}")
    ...
    def check_operands(self, operands, format_, mnemonic):
        ...
        for i, op in enumerate(operands):
            ...
            if format_ == 'R' and i == 0:  # Destination register
                if op_type != 'register':
                    self.errors.append(f"{mnemonic} destination must be register, got {op_type}")
                ...
                if op_type not in ['immediate', 'label']:
                    self.errors.append(f"{mnemonic} immediate operand expected, got {op_type}")
            elif format_ == 'V':  # Vector
                if op_type not in ['vector_register', 'memory']:
                    self.errors.append(f"{mnemonic} vector operand expected, got {op_type}")
            # Add more format checks

    def check_reversible_instruction(self, node):
        """Special checks for reversible instructions"""
        if self.current_function and self.current_function not in self.reversible_functions:
            self.warnings.append(f"Reversible instruction {node['mnemonic']} in non-reversible function {self.current_function}")
    ...
    def check_if(self, node):
        ...
        if cond_type != 'i32':  # Assuming boolean as i32 != 0
            self.warnings.append(f"If condition should be integer type, got {cond_type}")
    ...
    def check_return(self, node):
        ...
        if node['expression']:
            ...
            if self.current_function and self.symbol_table.get(self.current_function, {}).get('return_type') != expr_type:
                self.warnings.append(f"Return type mismatch in {self.current_function}: expected {self.symbol_table[self.current_function]['return_type']}, got {expr_type}")
    ...
    def check_binary_op(self, node):
        ...
        if left_type != right_type:
            self.warnings.append(f"Binary operation {op} operands have different types: {left_type} and {right_type}")

        # Specific op checks
        if op in ['+', '-', '*', '/', '%'] and left_type not in ['i32', 'i64', 'f32', 'f64']:
    ...
    def check_call(self, node):
        ...
        if len(args) != len(param_types):
            self.errors.append(f"Function {name} expects {len(param_types)} arguments, got {len(args)}")

        for i, arg in enumerate(args):
            if i < len(param_types):
                arg_type = self.infer_type(arg)
                if arg_type != param_types[i]:
                    self.warnings.append(f"Argument {i+1} type mismatch for {name}: expected {param_types[i]}, got {arg_type}")
    ...
    def infer_type(self, node):
        ...
        if node_type == 'literal':
            ...
            if self.current_function:
                local = self.symbol_table.get(self.current_function, {}).get('locals', {}).get(node['value'])
                if local:
                    return local
                param = self.symbol_table.get(self.current_function, {}).get('parameters', {}).get(node['value'])
                if param:
                    return param
            return 'i32'  # Default
        elif node_type == 'binary_op':
