...
class CRZCompiler:
    ...
    def parse(self, code):
        ...
        for l in lines:
            ...
            if m:
                ...
                continue
            if l == "}":
                instrs.append({"op": "END_FUNC"})
                continue
            # label?
            if re.match(r"^[A-Za-z_][A-Za-z0-9_]*:$", l):
                instrs.append({"op": "LABEL", "label": l[:-1]})
                continue
            ...
            if ins["op"] == "LOAD" and i + 1 < len(ast):
                ...
                if n["op"] == "ADD":
                    ...
                    if add_a == load_dst:
                        fused = {
                            "op": "FUSED_LOAD_ADD",
                            "args": [add_dst, ins["args"][1], add_b],
                            "raw": f"FUSED_LOAD_ADD {add_dst}, {ins['args'][1]}, {add_b}",
                        }
                        out.append(fused)
                        i += 2
                        continue
        ...
        for ins in ast:
            ...
            if op == "FUSED_LOAD_ADD":
                ...
                flat.append(
                    {
                        "op": "LOAD",
                        "args": [ins["args"][0], ins["args"][1], "/*fused_dst*/"],
                        "fused": True,
                    }
                )
                flat.append(
                    {
                        "op": "ADD",
                        "args": [ins["args"][0], ins["args"][0], ins["args"][2]],
                        "fused": True,
                    }
                )
                continue
    ...
    def compile(self, code):
        ...
        return {
            "ast": final,
            "warnings": warns,
            "passes": ["fusion", "reversible_emulation"],
        }
