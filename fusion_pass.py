...
class FusionPass:
    ...
    def try_fuse(self, statements, start_idx):
        ...
        for fused_name, pattern in self.fused_instructions.items():
            if self.match_pattern(statements, start_idx, pattern):
                return self.create_fused_instruction(
                    fused_name, statements[start_idx : start_idx + len(pattern)]
                )
        ...
        if start_idx + 1 < len(statements):
            ...
            if (
                ...
                and second["type"] == "instruction"
                and self.has_fusion_attribute(first)
                and self.can_fuse_pair(first, second)
            ):
    ...
    def has_fusion_attribute(self, instruction):
        ...
        return "attributes" in instruction and any(
            attr["name"] == "fusion" for attr in instruction["attributes"]
        )
    ...
    def can_fuse_pair(self, first, second):
        ...
        if first_mnemonic == "LOAD" and second_mnemonic in ["ADD", "SUB", "MUL"]:
            ...
            second_operands = [
                op["value"] for op in second["operands"] if "value" in op
            ]
