...
class CRZLexer:
    ...
    def tokenize(self):
        while self.pos < len(self.code):
            ...
            elif self.code[self.pos].isdigit() or (self.code[self.pos] == '-' and self.pos + 1 < len(self.code) and self.code[self.pos + 1].isdigit()):
                self.tokenize_number()
            elif self.code[self.pos] == '"':
                self.tokenize_string()
            elif self.code[self.pos] == '/' and self.pos + 1 < len(self.code) and self.code[self.pos + 1] == '/':
                self.tokenize_comment()
            elif self.code[self.pos] in '+-*/%=&|^< >!.':
                self.tokenize_operator()
            elif self.code[self.pos] in '(){}[],;:':
                self.tokens.append(Token(self.code[self.pos], self.code[self.pos], self.line))
                self.pos += 1
            else:
                raise SyntaxError(f"Unexpected character '{self.code[self.pos]}' at line {self.line}")
        return self.tokens
    ...
    def tokenize_operator(self):
        if self.code[self.pos] == '.' and self.pos + 1 < len(self.code) and self.code[self.pos + 1] == '.':
            ...
            self.tokens.append(Token('OPERATOR', self.code[self.pos:self.pos+2], self.line))
            self.pos += 2
        else:
...
class CRZParser:
    ...
    def parse_function(self):
        ...
        self.consume(expected_value=')')
        return_type = None
        if self.peek() and self.peek().value == '-' and self.pos + 1 < len(self.tokens) and self.tokens[self.pos + 1].value == '>':
            self.consume(expected_value='-')
            self.consume(expected_value='>')
        ...
        return {'type': 'function', 'name': name, 'parameters': params, 'return_type': return_type, 'body': body, 'attributes': func_attrs}
    ...
    def parse_statement(self):
        ...
        elif self.peek().type == 'IDENTIFIER' and self.pos + 1 < len(self.tokens) and self.tokens[self.pos + 1].value == ':':
    ...
    def parse_declaration(self):
        ...
        self.consume(expected_value=';')
        return {'type': 'declaration', 'name': name, 'var_type': type_, 'expression': expr}
    ...
    def parse_for(self):
        ...
        body = self.parse_block()
        return {'type': 'for', 'variable': var, 'start': start, 'end': end, 'body': body}
    ...
    def parse_operand(self):
        if self.peek().type == 'IDENTIFIER' and self.peek().value.startswith('R'):
            ...
            return {'type': 'vector_register', 'value': self.consume('IDENTIFIER').value}
